...
export default function MainLayout() {
    ...
    const { setIncomingAudioCall, answerCall, incomingAudioCall } = useContext(AudioContext)
    ...
    useEffect(() => {
        ...
        ws.on('incoming-call', async (data: { senderId: string, receiverId: string, senderName: string, from: string }) => {
            ...
            <div className="min-h-screen bg-secondary">
                ...
                {incomingCalling && (
                    <div className="mx-auto w-fit">
                        <div className={`fixed z-[100] ${incomingCalling ? 'visible opacity-100' : 'invisible opacity-0'} inset-0 grid place-items-center bg-black/20 backdrop-blur-sm duration-100 dark:bg-transparent`}>
                            <div onClick={(e_) => e_.stopPropagation()} className={`absolute max-w-xl rounded-lg  drop-shadow-lg dark:bg-zinc-900 dark:text-white ${incomingCalling ? 'opacity-1 duration-300' : 'scale-110 opacity-0 duration-150'} rounded border border-[#7808B1] max-w-6xl mx-auto bg-[#393B4C] text-white `}>
                                ...
                                <section className=''>
                                    <div className="bg-white p-7 flex justify-between items-center  rounded-lg shadow-lg">
                                        ...
                                        <button onClick={() => setIncomingCalling(null)} className='h-9 px-3 rounded-full p-2 bg-red-600'>Reject  </button>
